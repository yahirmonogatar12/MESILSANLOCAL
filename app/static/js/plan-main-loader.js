...
(function() {
    ...
    function configurarEventos() {
        ...
        document.body.addEventListener('click', async function(e) {
            ...
            // Botón Consultar
            if (target.id === 'btnConsultar-Control de operacion de linea Main' || 
                target.closest('#btnConsultar-Control de operacion de linea Main')) {
                e.preventDefault();
                mostrarIndicadorCarga(true);
            ...
            // Botón Exportar Main
            if (target.id === 'btnExportarMain-Control de operacion de linea Main' || 
                target.closest('#btnExportarMain-Control de operacion de linea Main')) {
                e.preventDefault();
                try { exportarTablaFiltrada(); } catch (err) { console.error('Error exporting:', err); }
                return;
            }
            
            // Botón Iniciar
            if (target.id === 'btnIniciar-Control de operacion de linea Main' || 
                target.closest('#btnIniciar-Control de operacion de linea Main')) {
                e.preventDefault();
                await actualizarStatus('EN PROGRESO');
                return;
            }
            
            // Botón Pausa
            if (target.id === 'btnPausa-Control de operacion de linea Main' || 
                target.closest('#btnPausa-Control de operacion de linea Main')) {
                e.preventDefault();
                crearModalMotivo({
                    titulo: 'Motivo de Pausa',
                    etiqueta: 'Describe la causa de la pausa:',
                    onConfirm: (motivo, cerrar) => { 
                        cerrar(); 
                        actualizarStatus('PAUSADO', { pause_reason: motivo }); 
                    }
                });
                return;
            }
            ...
            // Botón Fin
            if (target.id === 'btnFin-Control de operacion de linea Main' || 
                target.closest('#btnFin-Control de operacion de linea Main')) {
                e.preventDefault();
                const loteInput = document.getElementById("numeroLote-Control de operacion de linea Main");
                const lotNo = loteInput?.value?.trim();
                if (!lotNo) { 
                    await actualizarStatus('TERMINADO'); 
                    return; 
                }
                ...
                if (qty > 0 && producido < qty) {
                    crearModalMotivo({
                        ...
                        onConfirm: (motivo, cerrar) => { 
                            cerrar(); 
                            actualizarStatus('TERMINADO', { end_reason: motivo }); 
                        }
                    });
                } else {
                    await actualizarStatus('TERMINADO');
            ...
            // Botones BOM
            if (target.id === 'btnExportarBOM-Control de operacion de linea Main') {
                e.preventDefault();
                // Exportar BOM logic aquí
                return;
            }
            if (target.id === 'btnVerifPass-Control de operacion de linea Main') {
                e.preventDefault();
                // Verificar Pass logic aquí
                return;
            }
            if (target.id === 'btnRegistroMat-Control de operacion de linea Main') {
                e.preventDefault();
                // Registro material sustituto logic aquí
                return;
            }
            if (target.id === 'btnActivarComb-Control de operacion de linea Main') {
                e.preventDefault();
                // Activar combinación logic aquí
                return;
            }
            
            // Botón Exportar Historial
            if (target.id === 'btnExportarHistorial-Control de operacion de linea Main') {
                e.preventDefault();
                // Exportar historial logic aquí
                return;
            }
        });
        ...
        document.body.addEventListener('change', async function(e) {
            ...
            // Selector de línea
            if (target.id === 'linea-Control de operacion de linea Main') {
                mostrarIndicadorCarga(true);
                const filtros = obtenerFiltrosActuales();
                await cargarDatosPlanMain(filtros);
                return;
            ...
            // Checkbox "seleccionar todo" en thead
            if (target.type === 'checkbox' && target.closest('thead')) {
        ...
        document.body.addEventListener('keypress', async function(e) {
            ...
            if (target.id === 'numeroLote-Control de operacion de linea Main' && e.key === 'Enter') {
                e.preventDefault();
                mostrarIndicadorCarga(true);
                const filtros = { q: target.value };
                await cargarDatosPlanMain(filtros);
                return;
            }
        });
    ...
    /**
     * Crear modal para pedir motivo (pausa, terminación incompleta)
     */
    ...
    /**
     * Configurar estado de carga de botones de acción
     */
    ...
    async function actualizarStatus(nuevo, extraPayload = {}) {
        ...
        try {
            ...
            if (!contentType || !contentType.includes('application/json')) {
                ...
                mostrarError('Error: El servidor no respondió correctamente. Posiblemente la sesión expiró.');
                return;
            }
            ...
            console.error('Excepción actualizando status:', e);
            if (e.message && e.message.includes('JSON')) {
                mostrarError('Error: Respuesta inválida del servidor. Verifica tu sesión.');
            } else {
                mostrarError('Error de conexión al actualizar el status.');
            }
        } finally {
