...
let STORAGE_BOXES=[];let SELECTED_STORAGE=null;
...
<td class="mm-num">${fmtInt(r.allowance)}</td><td title="${r.model_name??''}">${(r.model_name??'').slice(0,38)}${(r.model_name||'').length>38?'…':''}</td>
...
function saveStorageChoice(){if(!SELECTED_STORAGE){showToast('Selecciona una caja de almacenamiento');return}console.log('Storage choice saved:', SELECTED_STORAGE);$('#mm-storage_box').value=SELECTED_STORAGE.management_no||'';closeStorageModal();showToast('Caja seleccionada: '+(SELECTED_STORAGE.management_no||'Sin número')+' - Se marcará como Ocupado al guardar')}
...
// Teardown del módulo MetalMask: quitar listeners, cerrar y remover overlays propios
