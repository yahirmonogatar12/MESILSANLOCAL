    ...
    if (groupedPlansData.length === 0) {
      ...
      rows.forEach((row, index) => {
        ...
        const planData = {
          ...
          process: cells[11]?.textContent?.trim() || '',
          ct: cells[12]?.textContent?.trim() || '',
          uph: cells[13]?.textContent?.trim() || '',
          ...
          tiempo_produccion: cells[19]?.textContent?.trim() || '',
          inicio: cells[20]?.textContent?.trim() || '',
          fin: cells[21]?.textContent?.trim() || '',
          grupo: grupoVisual, // Usar el grupo visual real en lugar del campo de celda
          extra: cells[22]?.textContent?.trim() || '',
          groupIndex: Math.floor(index / 5)
        };
