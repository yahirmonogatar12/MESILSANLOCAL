...
class SMTCSVHandler:
    ...
    def get_historial_data(self, filters=None):
        ...
        try:
            ...
            query += " ORDER BY scan_date DESC, scan_time DESC LIMIT 5000"
        ...
        try:
            ...
            insert_query = """
                ...
                (scan_date, scan_time, slot_no, result, part_name, quantity,
                 vendor, lot_no, barcode, feeder_base, previous_barcode,
                 source_file, line_number, mounter_number, file_hash)
    ...
    def parse_csv_row(self, row, filename, file_hash, line_number, mounter_number):
        ...
        try:
            ...
            part_name = row[4] if len(row) > 4 else ''
            quantity = self.parse_int(row[5]) if len(row) > 5 else 0
            vendor = row[6] if len(row) > 6 else ''
            lot_no = row[7] if len(row) > 7 else ''
            barcode = row[8] if len(row) > 8 else ''
            feeder_base = row[9] if len(row) > 9 else ''
            previous_barcode = row[10] if len(row) > 10 else ''
            
            return (scan_date, scan_time, slot_no, result, part_name, quantity,
                   vendor, lot_no, barcode, feeder_base, previous_barcode,
                   filename, line_number, mounter_number, file_hash)
