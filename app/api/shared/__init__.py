...
obtener_fecha_hora_mexico, _cuchillas_rows_to_json) se resuelven de forma
LAZY via PEP 562 `__getattr__`. Esto evita el import circular cuando
`routes.py` importa submódulos de `app.api.shared.*` durante su propia
carga (los submódulos son seguros porque no dependen de `routes`; solo
los re-exports lo hacen, y al ser lazy se evaluan cuando `routes` ya
terminó de definir esos símbolos).
"""
