"""Historial paginado de produccion SMT desde ``input_smt``.

Modulo de solo lectura gemelo del historial de ensamble. ``ts`` se separa en
Fecha y Hora; ``line`` se expone como Linea y no existe columna Barcode.
"""
...
bp = Blueprint("control_resultados_historial_produccion_smt", __name__)

PERMISO_PAGINA = "LISTA_DE_CONTROL_DE_RESULTADOS"
PERMISO_SECCION = "Consultar resultados"
PERMISO_BOTON = "Historial de produccion SMT"
...
@bp.route("/control_resultados/historial_produccion_smt")
@login_requerido
@_requiere_permiso
def historial_produccion_smt_ajax():
    try:
        ...
        logger.exception("Error cargando Historial de produccion SMT: %s", exc)
        return "Error al cargar el contenido", 500


@bp.route("/api/control_resultados/historial_produccion_smt", methods=["GET"])
@login_requerido
@_requiere_permiso
def api_historial_produccion_smt():
    try:
        page, per_page = _pagination_args()
        where_sql, params = _build_filters()

        count_sql = f"SELECT COUNT(*) AS n FROM input_smt WHERE {where_sql}"
        ...
        logger.exception("Error consultando input_smt: %s", exc)
        return jsonify(
            {
                "success": False,
                "error": "No fue posible consultar el historial de input SMT",
                "rows": [],
            }
        ), 500
...
@login_requerido
@_requiere_permiso
def export_historial_produccion_smt():
    """Exportar a Excel la pagina visible de input_smt, sin Barcode."""
    try:
        page, per_page = _pagination_args()
        where_sql, params = _build_filters()
        offset = (page - 1) * per_page
        ...
        logger.exception("Error exportando input_smt a Excel: %s", exc)
        return jsonify(
            {
                "success": False,
                "error": "No fue posible exportar el historial de input SMT",
            }
        ), 500
