    ...
    try:
        ...
        for r in rows:
            data.append({
                ...
                'uph': r.get('uph') if isinstance(r, dict) else r[12],
                'plan_count': r.get('plan_count') if isinstance(r, dict) else r[13],
                'produced_count': r.get('produced_count') if isinstance(r, dict) else r[14],
                'output': r.get('output') if isinstance(r, dict) else r[15],
                'entregadas_main': r.get('entregadas_main') if isinstance(r, dict) else r[16],
...
def api_plan_imd_create():
    ...
    try:
        ...
        if not (working_date and part_no and line):
            return jsonify({'error': 'Parámetros requeridos: working_date, part_no, line'}), 400
        ...
        params = (lot_no, wo_code, po_code, fecha, line, shift, model_code, part_no, project, process, plan_count, ct, uph, group_no, sequence)
...
def api_plan_imd_update():
    ...
    try:
        ...
        allowed_fields = ['status', 'plan_count', 'output', 'line', 'shift', 'working_date', 
                          'model_code', 'part_no', 'project', 'process', 'ct', 'uph', 'group_no', 'sequence']
...
def api_plan_imd_export_excel():
    ...
    try:
        ...
        ws = wb.active
        ws.title = 'Plan IMD'
        headers = ['Sec', 'LOT NO', 'WO', 'PO', 'Fecha', 'Línea', 'Shift', 'Modelo', 'Part No', 'Proyecto', 'Proceso', 'CT', 'UPH', 'Plan', 'Output', 'Status', 'Tiempo', 'Inicio', 'Fin', 'Grupo']
        ws.append(headers)
        for c in ws[1]:
            c.font = Font(bold=True)
            c.alignment = Alignment(horizontal='center')
        for p in plans:
            ...
            ws.append([
                p.get('secuencia',''), p.get('lot_no',''), p.get('wo_code',''), p.get('po_code',''), p.get('working_date',''),
                p.get('line',''), p.get('shift',''), p.get('model_code',''), p.get('part_no',''), p.get('project',''), p.get('process',''),
                p.get('ct',''), p.get('uph',''), p.get('plan_count',''), p.get('output',''), p.get('status',''), p.get('tiempo_produccion',''), p.get('inicio',''), p.get('fin',''), p.get('grupo','')
            ])
        bio = io.BytesIO()
        wb.save(bio)
        bio.seek(0)
        ts = datetime.utcnow().strftime('%Y%m%d_%H%M')
        return send_file(bio, as_attachment=True, download_name=f'Plan_IMD_{ts}.xlsx', mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def api_plan_imd_import_excel():
    ...
    try:
        ...
        working_date_default = request.form.get('working_date') or datetime.utcnow().strftime('%Y-%m-%d')
        ...
        line_reverse_map = {
            'PANA A': 'P1', 'PANA B': 'P2', 'PANA C': 'P3', 'PANA D': 'P4'
        }
        ...
        has_headers = first_val in ('line', 'linea', 'línea', 'lÃ­nea', 'part_no', 'part no')
        ...
        for _, row in df.iterrows():
            if has_headers:
                ...
                part_no = str(row.get('part_no', row.get('partno', row.get('part', '')))).strip()
                shift = str(row.get('shift', row.get('turno', 'DIA'))).strip().upper()
                try:
                    plan_count = int(float(row.get('plan_count', row.get('plan', row.get('qty', row.get('cantidad', 0)))) or 0))
                except Exception:
                    plan_count = 0
            else:
            ...
            # Saltar filas vacias
            if not part_no or part_no.lower() == 'nan' or not line_raw or line_raw.lower() == 'nan':
                continue
        ...
        return jsonify({'success': True, 'imported': imported, 'message': f'{imported} planes importados'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def api_plan_smt_create():
    ...
    try:
        ...
        if not (working_date and part_no and line):
            return jsonify({'error': 'Parametros requeridos: working_date, part_no, line'}), 400
...
def api_plan_smt_export_excel():
    ...
    try:
        ...
        ws = wb.active
        ws.title = 'Plan SMT'
        headers = ['Sec', 'LOT NO', 'WO', 'PO', 'Fecha', 'Linea', 'Shift', 'Modelo', 'Part No', 'Proyecto', 'Proceso', 'CT', 'UPH', 'Plan', 'Output', 'Status']
        ws.append(headers)
        for c in ws[1]:
            c.font = Font(bold=True)
            c.alignment = Alignment(horizontal='center')
        for p in plans:
            ...
            ws.append([
                p.get('secuencia',''), p.get('lot_no',''), p.get('wo_code',''), p.get('po_code',''), p.get('working_date',''),
                p.get('line',''), p.get('shift',''), p.get('model_code',''), p.get('part_no',''), p.get('project',''), p.get('process',''),
                p.get('ct',''), p.get('uph',''), p.get('plan_count',''), p.get('output',''), p.get('status','')
            ])
        bio = io.BytesIO()
        wb.save(bio)
        bio.seek(0)
        ts = datetime.utcnow().strftime('%Y%m%d_%H%M')
        return send_file(bio, as_attachment=True, download_name=f'Plan_SMT_{ts}.xlsx', mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def api_plan_smt_import_excel():
    ...
    try:
        ...
        line_reverse_map = {
            'SMT A': 'SA', 'SMT B': 'SB', 'SMT C': 'SC', 'SMT D': 'SD', 'SMT E': 'SE'
        }
        line_reverse_map_upper = {k.upper(): v for k, v in line_reverse_map.items()}
